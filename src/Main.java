...
public class Main {
    ...
    public static void main(String args[] ) throws SQLException, ClassNotFoundException {
       if(args.length != 3) {
           ...
           System.out.println();
           System.out.println("If left blank, the full benchmark (all 22 queries) will run on 3 client threads with 10 iterations each");
       } else {
           QUERY_SELECTION = args[0];
           NUMBER_OF_ITERATIONS = Integer.parseInt(args[1]);
           NUMBER_THREADS = Integer.parseInt(args[2]);
       }
        PGConnectionPoolDataSource pool = new PGConnectionPoolDataSource();
        pool.setDatabaseName("nina");
        pool.setUser("postgres");

        Person[] person = new Person[NUMBER_THREADS];
